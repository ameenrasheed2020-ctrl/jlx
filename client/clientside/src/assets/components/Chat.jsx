...
const Chat = () => {
    ...
    useEffect(() => {
        const fetchChat = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:6500/chat/conversation/${id}`);
                if (response.ok) {
    ...
    const sendMessage = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:6500/chat/conversation/${id}/messages`, {
                method: "POST",
    ...
    const updateDealStatus = async (dealStatus) => {
        try {
            const response = await fetch(`http://localhost:6500/chat/conversation/${id}/deal`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
